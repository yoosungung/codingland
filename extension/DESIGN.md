...
| `host/src/canvasViewDelta.ts` | zoom + `MAX_CANVAS_NODES` cap + `truncated` (`pushDelta` / `applyWorkspaceDelta` 공유) |
...
npm test             # core Jest + host Jest (EH-free merge/cap) + xvfbGuard/packageScripts node:test
npm run compile      # core dist/ then host out/
npm run ci           # compile + npm test + test:vscode (AA clean_code; EH soft-skip without xvfb)
...
QA 게이트(테넌트): 저장소 루트 [`.factory/quality.yaml`](../.factory/quality.yaml) `e2e.command` → `npm --prefix extension run test:vscode` (브라우저 `base_url` 아님). `test:vscode`는 **core를 먼저** 컴파일한 뒤 host를 빌드한다(클린 sync에서 `@codingland/core` 해석 실패 방지). AA 주간 NF: 같은 파일 `clean_code.command` → `npm --prefix extension run ci` — `ci`도 **compile 후** unit(`npm test`)·EH를 돌린다(install-only/NF에서 host Jest TS2307 방지; `ci`/`test:vscode` 키 유지). 시나리오 메모: [`e2e/scenarios/`](../e2e/scenarios/).
